...
export async function handleTribunalSourceSync(payload: TribunalSourceSyncPayload) {
  const run = await jobRunService.start({
    ...
    metadata: {
      ...
      genericTribunalImportLimit: payload.genericTribunalImportLimit ?? null,
      tjbaPageSize: payload.tjbaPageSize ?? null,
      tjbaMaxPages: payload.tjbaMaxPages ?? null,
      tjbaImportLimit: payload.tjbaImportLimit ?? null,
      tjesDebtorLimit: payload.tjesDebtorLimit ?? null,
      tjesPageSize: payload.tjesPageSize ?? null,
      tjesMaxPagesPerDebtor: payload.tjesMaxPagesPerDebtor ?? null,
      tjesImportLimit: payload.tjesImportLimit ?? null,
      tjmaYears: payload.tjmaYears ?? null,
      tjmaKinds: payload.tjmaKinds ?? null,
      tjmaLimit: payload.tjmaLimit ?? null,
      tjmaImportLimit: payload.tjmaImportLimit ?? null,
