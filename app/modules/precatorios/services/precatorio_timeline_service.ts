...
class PrecatorioTimelineService {
  async build(tenantId: string, assetId: string) {
    const asset = await precatorioRepository.findForTimeline(tenantId, assetId)
    const processMatchCandidates = await processMatchCandidateRepository.listForAsset(
      tenantId,
      assetId,
      50
    )
