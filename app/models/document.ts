...
export default class Document extends BaseModel {
  ...
  static boot() {
    ...
    this.before('create', (model: Document) => {
      if (!model.tenant_id) {
   ...
   * @example Document.query().withScopes((scopes) => scopes.forTenant(tenantId))
   */
   ...
   * @example Document.query().withScopes((scopes) => scopes.withoutTenantScope())
   */
  ...
  static withClient = scope((query) => {
    return (query as any).preload('client')
  })
  ...
  static withUploader = scope((query) => {
    return (query as any).preload('uploader')
  })
