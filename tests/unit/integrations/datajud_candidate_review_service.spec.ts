...
test.group('DataJud candidate review service', () => {
  test('accepts a high-score candidate and promotes it to a judicial process', async ({
    ...
    assert.equal(await countEvents(tenant.id, 'datajud_candidate_accepted'), 1)
  ...
  test('rejects a candidate without creating a judicial process', async ({ assert }) => {
    ...
    assert.equal(await countEvents(tenant.id, 'datajud_candidate_rejected'), 1)
...
async function countEvents(tenantId: string, eventType: string) {
  const [result] = await AssetEvent.query()
