...
test.group('DataJud public API adapter', () => {
  ...
  test('stores source records and upserts judicial processes by CNJ number', async ({ assert }) => {
    ...
    assert.equal(judicialProcess.court.code, 'TJDFT')
    assert.equal(judicialProcess.court.name, '3a VARA CIVEL DE CEILANDIA')
    assert.equal(judicialProcess.degree, 'G1')
    assert.equal(judicialProcess.secrecyLevel, 0)
    assert.equal(judicialProcess.judicialSystem.code, 1)
    assert.equal(judicialProcess.judicialSystem.name, 'PJe')
    assert.equal(judicialProcess.format.code, 1)
    assert.equal(judicialProcess.format.name, 'Eletrônico')
    assert.equal(judicialProcess.judicialClass.code, 7)
    assert.equal(judicialProcess.judicialClass.name, 'Procedimento Comum Cível')
    assert.equal(judicialProcess.judgingBody.code, '43179')
    assert.equal(judicialProcess.judgingBody.name, '3a VARA CIVEL DE CEILANDIA')
