...
test.group('Files upload', (group) => {
  ...
  test('should upload a file with authentication', async ({ client, assert }) => {
    ...
    const testFilePath = join(tmpDir, `test.txt`)
  ...
  test('should upload an image file', async ({ client, assert }) => {
    ...
    const tmpDir = app.tmpPath()
    await fs.promises.mkdir(tmpDir, { recursive: true })
    const testFilePath = join(tmpDir, `test.png`)
    const pngBuffer = Buffer.from(
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==',
      'base64'
    )
    await fs.promises.writeFile(testFilePath, pngBuffer)
  ...
  test('should validate file size', async ({ client }) => {
    ...
    const tmpDir = app.tmpPath()
    await fs.promises.mkdir(tmpDir, { recursive: true })
    const testFilePath = join(tmpDir, `test-${cuid()}.txt`)
  ...
  test('should validate file extensions', async ({ client }) => {
    ...
    const tmpDir = app.tmpPath()
    await fs.promises.mkdir(tmpDir, { recursive: true })
    const testFilePath = join(tmpDir, `test-${cuid()}.exe`)
    await fs.promises.writeFile(testFilePath, 'malicious content')
  ...
  test('should handle multiple file uploads', async ({ client, assert }) => {
    ...
    const tmpDir = app.tmpPath()
    await fs.promises.mkdir(tmpDir, { recursive: true })
    const testFile1Path = join(tmpDir, `test1-${cuid()}.txt`)
    const testFile2Path = join(tmpDir, `test2-${cuid()}.txt`)
    ...
    // Clean up test files
    await fs.promises.unlink(testFile1Path).catch(() => {})
    await fs.promises.unlink(testFile2Path).catch(() => {})
  })
  ...
  test('should categorize files correctly', async ({ client, assert }) => {
    ...
    for (const testFile of testFiles) {
      const testFilePath = join(tmpDir, testFile.name)
